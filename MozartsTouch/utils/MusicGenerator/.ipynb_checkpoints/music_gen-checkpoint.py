...
class MusicGen:
    def __init__(self, model_name = "musicgen-large") -> None:
        model_path = "/root/autodl-tmp/"
        self.processor = AutoProcessor.from_pretrained(model_path + model_name)
        self.model = MusicgenForConditionalGeneration.from_pretrained(model_path + model_name).to(device)
